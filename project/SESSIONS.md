...
**Evidence:** the pure-TypeScript `EnglishSliceCompletionStore` accepts only authoritative completed A4 state for `english-first-words@1`, emits one frozen idempotent receipt (`a6:english-first-word-complete`), and rejects running or foreign experience state. `EnglishLessonFlow.finish()` grants only after A4 performs the final authored transition; replay returns the same receipt, while re-entry observes prior completion without introducing A8 XP/economy or A11 persistence. The Hub derives English's visible subtitle from the same slice state (`First word learned ✓`), and a fresh English lesson renders a welcome-back state. Deterministic tests cover no early grant, forged/foreign-state rejection, exactly-once replay, and re-entry recognition; Playwright desktop/mobile evidence captures Hub before, Hub after, and completed-world re-entry. After fixing a test-only absolute dynamic-import type-resolution issue, CI `34554408723` passed quality (format, boundaries/typecheck, unit/integration tests, production build), browser stage-gate including the new visual evidence, and Android debug APK on implementation head `964fe06f`.
...
**Evidence:** `EnglishWorldScene` now guards asynchronous tutor completion with a scene lifecycle revision and exact session identity, disables learner controls during return, cancels the active tutor turn and interrupts the actor before navigation, and refuses late work from a disposed/inactive scene. Debug-only deterministic English fixtures exercise fail-once tutor, slow-once tutor, and fail-once speech behavior without any live provider. The responsive layout now treats short-height landscape as compact, tightens typography/spacing, keeps the companion and return control in-bounds, and preserves the existing desktop/mobile presentation. Dedicated Playwright coverage runs on desktop and mobile for tutor failure/recovery, speech failure containment, rapid exit during a slow tutor turn, repeated Hub ↔ English cycles, and a 900×500 short-landscape resize with captured visual evidence; existing authored asset-fallback and other-place smoke coverage remains green. The first evidence run exposed test assumptions that incorrectly expected Hub return to clear the selected English place; the evidence was corrected to model the authored `mode=english` return state rather than weakening runtime assertions. CI `34558753282` then passed quality (format, lint/typecheck-equivalent checks, unit/integration tests, production build), the full browser stage-gate including the new desktop/mobile resilience evidence, and Android debug APK on implementation head `efbe0001`.
